...
| Performance Lab (PERFORMANCE-LAB.md) | **MVP built 2026-07-24** — `/performance-lab` hub + Reaction / Recall / Track self-tests with meme tier ladders and Wordle-style share blocks; share links unfurl as `/api/arcade-card` score cards with the score + server-derived tier (§13 there). Deferred: percentiles/norms, Lab Score, leaderboards, daily circuit, dashboard metrics |
