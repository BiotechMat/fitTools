...
| Performance Lab (PERFORMANCE-LAB.md) | **Five stations live 2026-07-24** — `/performance-lab` hub + Reaction / Recall / Track (the range, ring-scored) / Vigil / Switch, all device-agnostic, meme tier ladders, share links unfurling as score cards with server-derived tiers; homepage hero CTA points here ("How fast are you?"). Steady built-then-pulled same day (scored the pointer, not the person — §13 there; /performance-lab/steady redirects). Bench: Wide Angle, Breathe. Deferred: percentiles/norms beyond Reaction's placeholder, Lab Score, leaderboards, daily circuit, dashboard metrics |
